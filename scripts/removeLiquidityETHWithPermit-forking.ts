...
const main = async() => {
    ...
    // Create a permit wallet with private key
    ...
    // Fund the permit wallet with ETH
    await owner.sendTransaction({
    ...
    // await USDC.transfer(impersonatedSigner.address, amountUSDC);
    // await USDC.approve(UNIRouter, amountUSDC);
    // await USDC.connect(owner).approve(UNIRouter, amountUSDC);

    // Add liquidity to the pool using th permit wallet
    await USDC.connect(impersonatedSigner).transfer(permitWallet.address, amountUSDC);

    // approve the router to spend money from the permit wallet
    await USDC.connect(permitWallet).approve(UNIRouter, amountUSDC);

    const addLiquidityTx = await ROUTER.connect(permitWallet).addLiquidityETH(
        USDCAddress,
        amountUSDC,
        amountTokenMin,
        amountETHMin,
    ...
    console.log(
        "=================Before removing liquidity with permit========================================"
    )
